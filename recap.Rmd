...
recap <- 
  ...
  str_detect(asset_name, "RESERVATION") ~ "No-Recap",
  str_detect(asset_name, "REWORK") ~ "No-Recap", 
  str_detect(asset_name, "MEDICAL") ~ "No-Recap",
  ...
  # str_detect(use_desc, "GOLF COURSE") ~ "No-Recap",
  ...
  rpuid %in% c(26688, 1296139, 24890, 30066, 30070, 27382, 31024, 32631, 35629)  ~ "No-Recap", # CPEN
...
#     x = "",
#     title = "Total Number of Scheduled Projects FY24-FY28")

# 
# fydp_recap_plan <- cat_c_recap %>% 
#   select(rpuid, op_activity) %>% 
#   group_by(rpuid, op_activity) %>% 
#   mutate(count = 1) %>%
#   mutate(op_activity = if_else(op_activity %in% c("PCS Lodging", "TDY Lodging"), "Commercial Recreation", op_activity)) %>% 
...
# 
# write_xlsx(fydp_recap_plan, "/Users/barnesj/OneDrive - USMC-MCCS.ORG/Documents/Data/Github_data/output/fydp_recap_plan")
