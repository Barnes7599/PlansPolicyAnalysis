...
#   select(company, org, updated_location_description, rpuid)
...
write_xlsx(joined_loc, "/Users/barnesj/OneDrive - USMC-MCCS.ORG/Documents/Data/Github_data/output/joined_loc.xlsx")
...
cat_c_recap <- cat_c_recap %>% 
  group_by(rpuid, fac_area_measure) %>% 
  mutate(count = n()) %>% 
  ungroup() %>% 
...
cat_c_recap %>% 
  select(region, installation, rpuid, op_activity, fci, fci_dt, build_dt, q_rating,net_profit, Asset) %>% 
  arrange(desc(net_profit), fci, fci_dt) %>% 
  filter(Asset == 1)

cat_c_recap %>% 
  select(region, installation, rpuid, op_activity, fci, fci_dt, build_dt, q_rating,net_profit, Asset) %>% 
  arrange(net_profit, fci, fci_dt) %>% 
  filter(Asset == 0)
