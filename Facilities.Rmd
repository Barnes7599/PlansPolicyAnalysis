...
saveRDS(
  read_xlsx(here::here("data", "MCCS_Facility_Inventory_Report.XLSX"),
    sheet = "Cat_code",
...
saveRDS(
  ...
  here::here("data", "fac.rds")
)

# Assign RDS data to variable
fac <- read_rds(here::here("data", "fac.rds"))
...
# Assign RDS data to variable
utl <- read_rds(here::here("data", "utl.rds"))
...
fac <- fac %>%
  ...
  #isolate use categroy code in own column called "usecode"
...
utl <- utl %>%
  ...
  group_by(RPUID) %>%
  mutate(
    use_pct_area = total_measure / sum(total_measure),
...
##### Cumulative Sqare Footage by Decade based on Facility Build Date
...
> *R & M*: The calculation in this report is based on 2.5% of the facilities Plant Replacement Value (PRV), this calculation assumes facility is fully recapitalizaed and that sustainment costs are being applied annualy to each facility.
