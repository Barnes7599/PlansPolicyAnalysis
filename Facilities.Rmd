...
df_clean <- fac_mapped %>%
 ...
 mutate(
    ...
    sus = if_else(naf_cat == "C", FY2024 * 2, FY2024),
...
region_cat_c <- df_final %>%
  ...
  gt() %>%
  tab_header(
    title = md("*FY24* Cat C MCCS FSRM Costs by Region"),
    subtitle = md("**iNFADS* data retrieved **Dec 28, 2021**")
  ) %>%
  fmt_currency(
    columns = c(total_sus, total_rm, total),
...
program <- df_final %>%
  ...
  gt() %>%
  tab_header(
    title = md("*FY24* MCCS FSRM Costs by Program"),
    subtitle = md("**iNFADS* data retrieved **Dec 28, 2021**")
  ) %>%
  fmt_currency(
    columns = c(total_sus, total_rm, total),
...
installation_cat_C <- df_final %>%
  ...
  gt() %>%
  tab_header(
    title = md("*FY24* Cat C MCCS FSRM Costs by Installation"),
    subtitle = md("**iNFADS* data retrieved **Dec 28, 2021**")
  ) %>%
  fmt_currency(
    columns = c(total_sus, total_rm, total),
...
df_final %>%
  filter(naf_cat == "C") %>% 
  summarise(
    total_rm = sum(RM_corrected)) #FV=PV(1+i)n
