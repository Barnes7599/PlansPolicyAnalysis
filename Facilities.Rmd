...
utl <- utl %>%
  ...
  ungroup() %>%
  mutate(use_pct_area = use_pct_area %>% replace_na(0)) %>%
  mutate(use_pct_other = if_else(use_pct_area == 0, 1, use_pct_area)) %>%
...
fac_mapped <- fac %>%
  inner_join(cat_codes, by = c("usecode" = "use_code")) %>%
  filter(naf_cat %in% c("A", "B", "C", "L"))

fac_mapped <- fac_mapped %>%
  inner_join(utl, by = "RPUID")
...
df_clean <- fac_mapped %>%
  ...
  mutate(RM_corrected = if_else(total_measure == 0,
    (RM * use_pct_other) / n,
    RM * use_pct_other
  ))
...
df_final <- df_clean %>%
  select(region, installation = installation.x, RPUID, usecode, usedesc, naf_cat, op_activity, fac_num, facility_desc, asset_name, fci, fci_dt, q_rating, build_dt, adq, sub, iadq, total_measure, use_pct_area, prv, RM, use_pct_other, RM_corrected, sus)

write_xlsx(df_final, path = "Data/df_finalv2.xlsx")
...
category <- df_final %>%
  group_by(RPUID) %>%
  mutate(RM_corrected = RM * use_pct_other) %>%
  group_by(naf_cat) %>%
  summarise(
    total_sus = sum(sus),
