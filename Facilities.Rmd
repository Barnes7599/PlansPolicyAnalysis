...
saveRDS(cat_code, "data/Cat_code.rds")

# Assign RDS data to variable
cat_code <- read_rds(here::here("data/", "Cat_code.rds"))


# Read in facility xlsx data and save data as RDS

fac <-  read_xlsx(path = "data/MCCS_Facility_Inventory_Report.XLSX",
    sheet = "Facility",
    skip = 1,
    col_types = c(
      "text", "text", "text",
...
utl <-  read_xlsx("data/MCCS_Facility_Inventory_Report.XLSX",
    sheet = "Utilization",
    skip = 1
  )
...
utl <- utl %>%
  ...
  filter(RPUID %notin% c(
    ...
    40532, 40526, 40534, 46374, 39594, 54613
  )) %>%
...
fac_mapped <- fac_mapped %>%
  ...
  filter(naf_cat %in% c("A", "B", "C", "L", "L (TDY)", "L (PCS)")) %>% 
  mutate(naf_cat = if_else(RPUID == 1173406, "L (PCS)", naf_cat))
...
gt_fsrm <- function(data, vbl){
 ...
 table_name <- colnames(table)[1]

 table_name_2 <- if_else(table_name %in% c("naf_cat"), "Category",
       if_else(table_name %in% c("region"), "Region",
           if_else(table_name %in% c("op_activity"), "Program","")))

 table %>% 
   gt() %>% 
 tab_header(
   title = md(str_glue("*Annual* MCCS FSRM Costs by {table_name_2}")),
   subtitle = md("*iNFADS* data retrieved **Jun 17, 2021**")
  ) %>%
  fmt_currency(
    columns = c(total_sus, total_rm, total),
...
```{r}
df_final %>% 
 filter(use_desc %in% c("GOLF COURSE")) %>% 
  select(Region = region, Installation = installation, Sustainment = sus, RM = RM_corrected) %>% 
  mutate(Total = Sustainment + RM,
         Seven_Yr_Total = Total * 7,
         Sustainment = Sustainment %>% scales::dollar(),
         RM = RM %>% scales::dollar(),
         Total = Total %>% scales::dollar(),
         Seven_Yr_Total = Seven_Yr_Total %>% scales::dollar())
...
df_final %>%
  ...
  arrange(naf_cat) %>% 
  mutate(APF = if_else(naf_cat == "A", total*.85,
                       if_else(naf_cat == "B", total*.65, 0))) %>% 
  mutate(NAF = if_else(naf_cat == "A", total*.15,
                       if_else(naf_cat == "B", total*.35, total))) %>% 
  janitor::adorn_totals()
